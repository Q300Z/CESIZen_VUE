    ...
    >
      <template #top>
        <v-toolbar flat>
          ...
          <v-spacer />
          <v-btn color="primary" to="/admin/emotions" :active="false">Liste des émotions</v-btn>
          <v-btn color="primary" to="/admin/emotions/base/add">Ajouter une émotion de base</v-btn>
        </v-toolbar>
      </template>

      <template #item.actions="{ item }">
        <v-btn :to="`/admin/emotions/edit/base/${item.id}`" icon variant="text">
          <v-icon>mdi-pencil</v-icon>
        </v-btn>
        <ModalConfirmation title="Confirmation de la suppression" @confirm="() => deleteEmotion(item)">
