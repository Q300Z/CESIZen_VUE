...
<template>
  <v-container>
    <div class="d-flex justify-center flex-column align-center ga-5">
      ...
      <h1>Bienvenue sur</h1>
      <v-img src="/cesizen_logo.png" width="200"></v-img>
      <h3 class="mt-5">Choisissez une activité</h3>
      <div class="d-flex justify-center ga-3 flex-wrap" >
      <v-card v-for="item in activity" :key="item.title" :disabled="!item.to" :title="item.title" min-width="375" :prepend-icon="item.icon" :subtitle="item.subtitle">
